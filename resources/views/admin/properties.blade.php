@php
    /* ─── Shared admin layout partial used via @include ─────────────────────── */
    /*  Views that @include this file must pass $stats array */
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Property Listings — Admin | BOUESTI</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* Copy admin shell styles (same as dashboard) */
        ...
        .col-img { width:52px;height:40px;object-fit:cover;border-radius:6px; }
        .col-title { font-weight:700;color:#192F59; }
        .col-sub { font-size:.76rem;color:#9ca3af;margin-top:.1rem; }
        .price { font-weight:700;color:#00A553; }
...
<div class="admin-shell">
    ...
    <aside class="admin-sidebar" id="sidebar">
        ...
        <a href="{{ route('admin.properties', ['status'=>'pending']) }}" class="sidebar-item {{ $status === 'pending' ? 'active' : '' }}">
            <span class="s-icon">⏳</span> Pending
            @if($counts['pending'] > 0)<span class="sidebar-badge">{{ $counts['pending'] }}</span>@endif
        </a>
        <a href="{{ route('admin.properties', ['status'=>'approved']) }}" class="sidebar-item {{ $status === 'approved' ? 'active' : '' }}">
            <span class="s-icon">✅</span> Approved
        </a>
        <a href="{{ route('admin.properties', ['status'=>'all']) }}" class="sidebar-item {{ $status === 'all' ? 'active' : '' }}">
        ...
        <a href="{{ route('profile.edit') }}" class="sidebar-item"><span class="s-icon">👤</span> My Profile</a>
    </aside>
        ...
        <div class="page-hdr">
            <div>
                <div class="page-hdr-title">
                    @if($status === 'pending') ⏳ Pending Listings
                    @elseif($status === 'approved') ✅ Approved Listings
                    @elseif($status === 'rejected') ✕ Rejected Listings
                    @else 🏠 All Property Listings
                    @endif
                </div>
                <div class="page-hdr-sub">{{ $properties->total() }} result(s) found</div>
            </div>
            <a href="{{ route('admin.dashboard') }}" class="btn btn-view">← Dashboard</a>
        </div>
        ...
        <div class="filter-row">
            <div class="filter-tabs">
                @foreach(['pending'=>'⏳ Pending','approved'=>'✅ Approved','rejected'=>'✕ Rejected','all'=>'All'] as $val=>$label)
                    <a href="{{ route('admin.properties', ['status'=>$val, 'q'=>$search]) }}"
                       class="filter-tab {{ $status === $val ? 'active' : '' }}">
                        {{ $label }}
                        @if(isset($counts[$val])) ({{ $counts[$val] }}) @endif
                    </a>
                @endforeach
            </div>
            <form class="filter-search" method="GET" action="{{ route('admin.properties') }}">
                <input type="hidden" name="status" value="{{ $status }}">
                <input type="text" name="q" value="{{ $search }}" placeholder="Search by title, area, address…">
        ...
        <div class="card">
            <div class="card-hdr">
                <h3>
                    ...
                    <span style="font-size:.78rem; font-weight:500; color:#9ca3af;">— {{ $properties->total() }} total</span>
                </h3>
            </div>
            <div class="tbl-wrap">
                @if($properties->isEmpty())
                    <div class="empty">
                        ...
                        <h3>
                            @if($status === 'pending') No Pending Listings — All Clear!
                            @else No Properties Found @endif
                        </h3>
                        <p>
                            @if($search) No results for "{{ $search }}". Try a different search.
                            @elseif($status === 'pending') All property submissions have been reviewed.
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="color:#9ca3af;font-size:.78rem;">{{ $loop->iteration + ($properties->currentPage()-1)*$properties->perPage() }}</td>
                            <td>
                                <div class="prop-cell">
                                    <img src="{{ $p->coverImageUrl() }}" alt="{{ $p->title }}" class="col-img">
                                    <div>
                                        <div class="col-title">{{ $p->title }}</div>
                                        <div class="col-sub">{{ Str::limit($p->address, 36) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>{{ $p->area }}</td>
                            ...
                            <td style="font-size:.82rem;color:#7a8ba3;white-space:nowrap;">{{ $p->distance_from_campus ?? '—' }}</td>
                            <td>
                                <div class="col-title" style="font-size:.85rem;">{{ $p->landlord->user->name ?? '—' }}</div>
                            </td>
                            <td style="color:#7a8ba3;font-size:.8rem;white-space:nowrap;">{{ $p->created_at->format('d M Y') }}</td>
                            <td>
                            ...
                            <td>
                                <div style="display:flex;gap:.35rem;flex-wrap:wrap;">
                                    ...
                                    <form method="POST" action="{{ route('admin.properties.approve', $p) }}" style="display:inline" onsubmit="return confirm('Approve \'{{ addslashes($p->title) }}\'?');">
                                        @csrf
                                    ...
                                    <button type="button" class="btn btn-reject"
                                        onclick="openRejectModal({{ $p->id }}, '{{ addslashes($p->title) }}')">
                                        ✕ Reject
                                    </button>
                                    @endif
                                </div>
...
<div class="modal-backdrop" id="rejectModal">
    <div class="modal">
        ...
        <p id="rejectModalDesc">Reject this listing?</p>
        <form id="rejectForm" method="POST">
            @csrf
            <textarea name="rejection_reason" id="rejectReason" placeholder="Reason for rejection (optional) — the landlord may see this…"></textarea>
...
function openRejectModal(id, title) {
    document.getElementById('rejectForm').action = `/admin/properties/${id}/reject`;
